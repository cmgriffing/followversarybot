...
const CLIENT_ID = "bd18p9eycculb64acrdjjdp72bt4xr";
...
export function startBot(
  ...
  (async function init() {
    ...
    client.on("message", async (channel, tags, message, self) => {
      ...
      if (cache.followers[tags.username || tags["user-id"] || ""]) {
        return;
      }

      // user_id=${tags["user-id"]}&
      const fetchUrl = `https://api.twitch.tv/helix/channels/followers?broadcaster_id=${broadcaster_id}&scope=moderator:read:followers`;
